...
_qunit2['default'].test('Check if we are choosing endTime rather than time if time property exists in parallel', function (assert) {
  var oldVttCue = window.VTTCue;
  var player = (0, _playerProxyMaker2['default'])();
  var setTracks = [];
  var cues = [];
  ...
  (0, _srcPlayItem2['default'])(player, null, {
    cuePoints: [{ time: 0, endTime: 0.0166, type: 'foo' }, { time: 1, endTime: 1.0166, type: 'bar' }]
  });

  assert.deepEqual(cues, [{ endTime: 0.0166, startTime: 0, type: 'foo' }, { endTime: 1.0166, startTime: 1, type: 'bar' }], 'We are not choosing the property endTime correctly');
